...
public func `weak`<WeakOwner>(_ owner: WeakOwner, closure: @escaping (WeakOwner) -> Void)
    -> () -> Void where WeakOwner: AnyObject {
        return weak(owner, ifNilJustReturn: (), closure: closure)
}

public func `weak`<WeakOwner, R>(_ owner: WeakOwner, ifNilJustReturn defaultValue: R, closure: @escaping (WeakOwner) -> R)
    -> () -> R where WeakOwner: AnyObject {
        return { [weak owner] in
            guard let strongOwner = owner else { return defaultValue }
...
public func `weak`<WeakOwner, P, R>(_ owner: WeakOwner, ifNilJustReturn defaultValue: R, closure: @escaping (WeakOwner, P) -> R)
